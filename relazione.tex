...
Poiché i nomi dei principi attivi possono essere molto lunghi e non hanno un formato standard, in PRINCIPIO-ATTIVO verrà creata una chiave numerica sequenziale chiamata \textit{cod\_pa} avrà un solo attributo,   TODO FINIRE!!! scelto anche come chiave primaria, chiamato \textit{Nome}, mentre alla relazione CONTIENE verrà aggiunto l'attributo \textit{Quantità}.
...
In realtà il secondo trigger protegge anche da eventuali aggiornamenti errati del paziente, impostando sempre il valore corretto.
\begin{lstlisting}
...
Si potrebbe pensare di disabilitare momentaneamente il trigger (o constraint), inserire il paziente, inserire il ricovero e poi riabilitarlo, ma questa soluzione è stata scartata in quanto non-safe.
...
Similmente a quanto detto prima è più grave che, dato una farmaco, non si sappiano quali principi attivi lo compongono, piuttosto che aver memorizzato principi attivi che non compaiono in alcun farmaco.
Nel blocco sottostante viene illustrata questa soluzione:

TODO manca l'inserimento di un nuovo farmaco... come fare a creare tupla in CONTIENE? non si può perché non conosco a priori. Sta all'utente

Sotto assicuro che ad un farmaco non vengano mai rimossi tutti i principi attivi
\begin{lstlisting}
...
\end{lstlisting}
La funzione \textit{ricalcolo\_gg} calcola la somma totale dei giorni che una paziente ha passato da ricoverato.
...
TODO trigger per creazione codici di diagnosi e terapia
TODO dire sopra nella crazione domini aggionrare blocco codice SQL
\cleardoublepage

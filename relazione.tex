...
\begin{itemize}
  ...
  \item Ha pochi valori duplicati in quanto avere tante entry ripetute vanifica il lavoro dell'indice perchè in esecuzione della ricerca si fa riferimento a più valori procedendo sequenzialmente.
  
  \item Se l'indice contiene solo il valore della colonna e la tabella a cui fa riferimento allora vi è la necessità di leggere dalla tabella i valori della tupla corrispondente. Se, invece, l'indice viene costrito su più colonne (quelle necessarie) della tabella non è richiesta una ulteriore lettura. Questa implementazione è possibile al costo dell'utilizzo di maggior spazio.
\end{itemize}
Gli indici possono essere costruiti basandosi su strutture dati differenti a seconda di come verrà sfruttato nelle query. Ad esempio se una colonna viene spesso utilizzata in test di uguaglianza una struttura con tabella di hash risulta più efficente di un btree che può essere utilizzato per ottimizzare i generici confronti. Inoltre, la caratteristica intrinseca degli indici di ordinare i record consente di ottimizzare quelle query in cui si richiede, tramite l'istruzione ORDER BY, di ordinare attraverso una determinata colonna. Con un indice il lavoro è già fatto e non viene svolto in tempo d'esecuzione. 

Per la valutazione sull'inserimento o meno di alcuni indici nel caso specifico del progetto è stata utilizzata l'istruzione EXPLAIN ANALYZE che, invece di restituire il risultato della query, restisce il piano dettagliato con cui il database procede nell'esecuzione dei comandi dell'interrogazione. In particolare consente di analizzare i costi, forniti tramite un valore numerico, di una specifica sequenza di operazioni nonchè il tempo impiegato nella stessa. E' interessante notare come nell'output dell'analisi si faccia riferimento al tempo di pianificazione della query oltre al tempo della sua esecuzione.

Per l'implementazione di un indice si è considerato il fatto che spesso si utilizzano il nome e/o il cognome del paziente per ricavare alcune informazioni dal database senza l'inserimento del codice fiscale. 

L'esecuzione di una semplice query di ricerca per l'ottenimento dei dati tramite l'inserimento del nome di un paziente risulta molto più efficente se supportata da un indice sul nome mentre non risulta essere influente durante le operazioni di inserimento e cancellazione.

L'esecuzione di una query con lo scopo di fornire tutte le coppie di pazienti con stesso nome e cognome, invece, non subisce miglioramenti delle performance quando si implementa un indice su (nome,cognome) e ciò è deducibile tramite  l'operazione di analisi che non mostra variazioni nel costo calcolato e nel tempo di esecuzione dell'interrogazione.

L'esecuzione di una query con l'obbiettivo di fornire tutti i dati relativi ai ricoveri di un determinato paziente 
...
\begin{lstlisting}
  ...
  WHERE nome = 'tino'AND cognome = 'labella';
\end{lstlisting}
viene resa più efficiente tramite la creazione di un indice su (nome,cognome). E' interessante notare come il tempo di pianificazione aumenta in presenza dell'indice mentre le effettive operazioni risultano più efficenti e veloci in termini di costo e tempo. 

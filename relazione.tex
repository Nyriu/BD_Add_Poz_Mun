...
4) Diagnosi x% TODO vincolo integrità Data deve essere contenuta in [D.Inizio, D.Fine] di RICOVERO
5) Effettuata durante x
6) Terapia
6.1) Medico
7) Farmaco
8) Somministra
...
di tale trasformazione è riportato in \autoref{fig:piccolo2}.  Ricordando
...
senza perdita d'informazione, e con l'aggiunta di una nuova entità, in tre
relazioni binarie.
...
\begin{tikzpicture} % TODO spostare dopo la spiegazione
  ...
  %\hspace*{-1cm} % tutto un po' più a  sinistra
  ...
  \node [
    ...
    (farmaco)
    {
     ...
     \nodepart{two} Dose\_Giornaliera\_Raccomandata
  ...
  \draw[-latex] (terapiaprescritta.one south) -- ++(0,-0.4) -| ($(terapiaprescritta.one south) + (11,0)$) |- ($(diagnosi.one south) + (-0.2,-0.50)$) -| ($(diagnosi.one south) + (-0.2,0)$);

  \draw[-latex] (terapiaprescritta.two south) -- ++(0,-0.2) -| ($(terapiaprescritta.two south) + (9,0)$) |- ($(terapia.one south) + (0,-0.50)$) -| ($(terapia.one south) + (0,0)$);
\end{tikzpicture}
...
Finalmente è possibile iniziare la traduzione del modello E-R in quello Relazionale.
Come prima cosa, in quanto immediato, ogni entità verrà rappresentata da una tupla.%TODO verifica correttezza 
Queste verranno poi modificate per poter mantenere anche le informazioni delle relazioni, può capitare che ad una relazione venga associata un nuova tupla.
...
% \vspace{5mm}
% \smallskip
% i nuovi valori \textit{Diagnosi} e \textit{Terapia}



%TERAPIA\_PRESCRITTA(\textit{\underline{Diagnosi}, \underline{Terapia}, Data\_Inizio, Data\_Fine,\\Medico\_Prescrivente})

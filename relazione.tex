...
La relazione di afferenza tra RICOVERO e PAZIENTE prende il nome di VIENE-RICOVERATO.
...
Un nuovo paziente viene inserito nella base di dati la prima volta in cui viene ricoverato, quindi la partecipazione di PAZIENTE alla relazione VIENE-RICOVERATO è totale.
...
La chiave di TERAPIA è composta dagli attributi \textit{Dose Giornaliera} e \textit{Modalità Somministrazione} e dalla relazione SOMMINISTRATO-DURANTE che lega FARMACO a TERAPIA.
...
È importate considerare il fatto che una terapia non può esistere senza un farmaco da somministrare, quindi la partecipazione di TERAPIA alla relazione SOMMINISTRATO-DURANTE è totale.
...
Nella terza fase della progettazione è stato modellato nello schema E-R l'ultimo punto dell'analisi dei requisiti attraverso la relazione MOTIVA-TERAPIA.
...
Da qui la necessità di omettere attributi data inizio, data fine e medico prescrivente in TERAPIA, assegnandoli alla relazione MOTIVA-TERAPIA.
...
Nei requisiti è specificato che per alcune patologie mancano le cure, per questo motivo la partecipazione a MOTIVA-TERAPIA di DIAGNOSI è parziale.
...
Nel complesso MOTIVA-TERAPIA, per ora, risulta essere una relazione Uno-A-Molti: nel testo è specificato che ad ogni diagnosi può essere associata al più una terapia.
...
\begin{itemize}
  \item in nessuna tripletta di MOTIVA-TERAPIA può comparire due volte la     stessa DIAGNOSI;
...
Ciò è stato fatto riportando rispettivamente le due informazioni nell'attributo \textit{Medico} in DIAGNOSI e nell'attributo \textit{Medico Prescrivente} in MOTIVA-TERAPIA.
...
La rimozione delle generalizzazioni non è sempre così immediata: se PAZIENTE-FUORI-REGIONE avesse avuto una relazione aggiuntiva rispetto al genitore si sarebbero dovute valutare varie opzioni.
...
Poiché i nomi dei principi attivi possono essere molto lunghi e non hanno un formato standard, in PRINCIPIO-ATTIVO, sono salvati nella variabile \textit{Nome}, mentre una sequenza numerica chiamata \textit{Codice Principio Attivo} viene usata come chiave primaria.
...
\begin{itemize}
  ...
  \item TERAPIA-PRESCRITTA non può essere in relazione \textit{È Effetto Collaterale} con una DIAGNOSI avente una data che precede la data della DIAGNOSI con cui definisce la sua chiave primaria;
  \item la data d'inizio della TERAPIA-PRESCRITTA deve essere compresa tra la data della DIAGNOSI che la definisce e la data della DIAGNOSI che identifica l'effetto collaterale;
  \item la data di fine della TERAPIA-PRESCRITTA non può precedere la data di inizio.
\end{itemize}
...
Ricordiamo che quest'ultima, quando si progetta una basi di dati, è da evitare, non tanto perché l'informazione ripetuta occupa spazio doppio, ma piuttosto perché si rischia di introdurre pericolose inconsistenze.
...
Il ciclo formato dalle entità DIAGNOSI, TERAPIA-PRESCRITTA e DIAGNOSI non crea ridondanze perché ogni elemento fornisce un'informazione unica: la prima diagnosi giustifica la cura, la terapia prescritta spiega quando tale cura è stata applicata, mentre la seconda diagnosi riporta eventuali effetti collaterali.
...
\begin{itemize}
	\item EFFETTUATA-A: una relazione Uno-A-Molti tra PAZIENTE e DIAGNOSI.
...
TUATA-A tra PAZIENTE e DIAGNOSI in funzione della frequenza delle operazioni di inserimento nuove diagnosi e stampa dello storico diagnosi di un paziente.
...
\end{lstlisting}
Il primo\footnote{per ragioni di semplicità i primi sei caratteri del codice fiscale possono essere anche consonanti}e l'ultimo rispettano le normali convenzioni mentre gli altri sono rappresenti da delle sequenze distinte di interi.
...
Annualmente a quanti giovani(sotto i 30) viene diagnosticato un tumore?
%\begin{figure}[!ht] % TODO abbellire
%  \centering
...
%\begin{figure}[!ht] % TODO abbellire
%  \centering
%  \includegraphics[width=\linewidth]{grafico_4}
%  \caption{grafico\_4}
%  \label{fig:grafico_4}
%\end{figure}


Rispetto alla durata media in giorni di un ricovero, per quanti giorni rimangono ricoverati i pazienti a cui è stato diagnosticato un tumore? 
%\begin{figure}[!ht] % TODO abbellire
%  \centering
...
Si vuole sapere com'è cambiata la diffusione dei farmaci negli anni, mostrando i dieci più utilizzati ed i dieci meno utilizzati.
%\begin{figure}[!ht] % TODO abbellire
%  \centering
%  \includegraphics[width=\linewidth]{grafico_6}
%  \caption{grafico\_6}
%  \label{fig:grafico_6}
%\end{figure}

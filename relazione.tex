...
SELECT *
FROM patologie p1
INNER JOIN patologie p2 ON p1.cod_pat = p2.cod_pat
WHERE p1.cf <> p2.cf 
AND ((p1.age < 50 AND p2.age > 50) OR (p1.age > 50 AND p2.age < 50));
\end{lstlisting}
...
INNER JOIN terapie_prescritte tp2
  ...
  AND tp2.data_i > tp1.data_i
WHERE tp1.cf = 'MIASOR20G71P162Y'
ORDER BY tp1.data_i DESC, tp2.data_i DESC;
...
WHERE EXISTS ( 
  ...
  WHERE tp1.diagnosi <> tp2.diagnosi
  AND tp1.paziente = tp2.paziente
  AND tp1.farmaco = tp2.farmaco
  AND tp2.data_i > tp1.data_i
  AND NOT EXISTS (
    SELECT *
    FROM terapie_prescritte tp3
    WHERE tp3.diagnosi <> tp2.diagnosi
    AND tp3.diagnosi <> tp1.diagnosi
    AND tp3.paziente = tp2.paziente
    AND tp3.farmaco = tp2.farmaco

...
Per prima cosa dovrà essere definita una tabella per ogni tupla dello schema poi si procederà alla creazione di funzioni, constraints e triggers per assicurare i vincoli d'integrità.
Si suppone che tutti i comandi SQL successivi vengano eseguiti all'interno dello schema \textit{ospedale}, cioè dopo l'esecuzione del seguente blocco:
\begin{lstlisting}
...
\begin{itemize}
  ...
  \item una tabella un po' più complessa che presenta anche un campo chiave esterna;
  \item ed infine una tabella che rappresenta un'entità debole, con chiave primaria una coppia;
\end{itemize}
